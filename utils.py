...
def visualize(epoch_id, machine_steering, out_dir, perform_smoothing=False,
    ...
    front_vid_path = join_dir(epoch_dir, 'epoch{:0>2}_front.mp4'.format(epoch_id))
    assert os.path.isfile(front_vid_path)
    
    dash_vid_path = join_dir(epoch_dir, 'epoch{:0>2}_dash.mp4'.format(epoch_id))
    ...
    out_path = join_dir(out_dir, 'epoch{:0>2}_human_machine.mp4'.format(epoch_id))
    ...
    for f_cur in range(len(machine_steering)):
        ...
        if (frame_count_limit is not None) and (frame_count_limit == 1):
            cv2.imwrite(out_path.replace('mp4', 'jpg'), fimg)
            sys.exit()
